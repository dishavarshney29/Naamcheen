...
const AddCrop = props => {
	...
	async function handleUpload(file) {
		try {
			...
			setQualityScore(round(response.data.qualityScore));
	...
	async function handleSubmit(e) {
		e.preventDefault();
		try {
			const response = await addListing(name, quantity, qualityScore, metadata.quality, metadata.url);
		...
		<div>
			...
			<form noValidate autoComplete="off" onSubmit={handleSubmit}>
				...
				{showQualityScore && (
					<>
						...
						<FormControl fullWidth variant="outlined" style={{ marginBottom: "20px" }}>
							<InputLabel htmlFor="outlined-text">Score</InputLabel>
							<OutlinedInput
								id="outlined-text"
								type="number"
								value={qualityScore}
				...
				<Button
					...
					variant="contained"
					color="secondary"
					disabled={!showQualityScore || name === "" || quantity === 0 || qualityScore === 0}
				>
					Submit
				</Button>
			</form>
